...
public class City{
    ...
    /**
     * fully featured constructor with parameters for an entirely customized city
     * @param name specifies name of the city
     * @param cityhall contains an entire CityHall object filled with Police objects
     * @param school contains an entire School object filled with Teacher and Kid objects
     * @param people contains all people roaming about town
     */
    ...
    void listCityHallPeople(){
        ...
        for (int i = 0; i < cityhall.population(); i++)
            System.out.println(cityhall.getOccupant(i).getRole() +" "+ cityhall.getOccupant(i).getName());
    }
    ...
    void payEmployees(){
        for (int i = 0; i < cityhall.population(); i++)
            cityhall.getOccupant(i).payEmployee();

        for(int i = 0; i < school.population(); i++){
            if(school.getOccupant(i) instanceof Teacher)
